    ...
    <div className="flex h-screen overflow-hidden">
      ...
      >
        <div className="flex flex-col h-full">
          <div className="p-5 border-b border-gray-800">
            <h1 className="text-xl font-bold text-brand-500">WA Bizz</h1>
            <div className="mt-1 flex items-center gap-2">
              <span
                className={`w-2 h-2 rounded-full ${waConnected === true ? 'bg-green-500' : waConnected === false ? 'bg-red-500' : 'bg-yellow-500'}`}
              />
              <span className="text-xs text-gray-400">
                {waConnected === true ? 'WhatsApp connected' : waConnected === false ? 'Disconnected' : 'Checking...'}
              </span>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        <header className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center gap-4 lg:hidden">
          ...
          <span className="font-bold text-brand-500">WA Bizz</span>
        </header>
